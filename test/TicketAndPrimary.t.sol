...
contract PrimaryMarketTest is Test {
    ...
    function testPurchaseTicket() public {
        ...
        ticketId = ticketId; // Silence unused variable warning
        ...
        // uint256 eventCreatorBalance = purchaseToken.balanceOf(eventCreator);
        // assertEq(eventCreatorBalance, ticketPrice, "Event creator's token balance should be increased by the ticket price.");

        // // Check that the ticket's price is set correctly
        // assertEq(ticketNFT.ticketPrice(), ticketPrice, "Ticket price should be set correctly.");

        // // Check that the ticket's event name is set correctly
        // assertEq(ticketNFT.eventName(), eventName, "Event name should be set correctly.");

        // // Check that the ticket's event creator is set correctly
        // assertEq(ticketNFT.creator(), eventCreator, "Event creator should be set correctly.");

        // // Check that the ticket's event max number of tickets is set correctly
        // assertEq(ticketNFT.maxNumberOfTickets(), maxTickets, "Max number of tickets should be set correctly.");
